...
### 📝 QR 생성/편집 화면 (CreateQRScreen) - v1.4 업데이트
...
#### 개인정보 수집 및 이용 동의 (v1.4 신규) ✨
- **최초 1회**: 신규 사용자가 첫 번째 QR코드 생성 시에만 표시
- **게스트 사용자**: 로컬 저장소(AsyncStorage)에 동의 상태 저장
- **로그인 사용자**: 서버 데이터베이스에 동의 상태 및 일시 저장
- **조건부 표시**: 이전에 동의한 사용자에게는 동의 UI를 표시하지 않음
- **법적 컴플라이언스**: 개인정보보호법 준수를 위한 최소한의 동의 절차
...
- **더보기 메뉴**: 로그인 사용자만 표시 (⋮ 아이콘)
  ...
  - **약관 및 동의 내역** (v1.4 준비): 개인정보 동의 상태 관리 화면 (향후 구현 예정)
- **데이터 구분**:
...
### Version 1.4.0 (2025-09-09) ✨
- **개인정보 수집 및 이용 동의 기능 구현**: 
  - 최초 1회 개인정보 동의 시스템 완전 구현
  - CreateQRScreen에서 신규 사용자 첫 QR 생성 시에만 동의 UI 표시
  - 게스트 사용자는 AsyncStorage, 로그인 사용자는 서버 DB에 동의 상태 저장
- **백엔드 API 확장**: 
  - `POST /auth/privacy-consent`: 개인정보 동의 상태 업데이트
  - `GET /auth/privacy-consent`: 개인정보 동의 상태 조회
  - Owner 엔티티에 `isPrivacyConsentGiven`, `privacyConsentDate` 필드 추가
- **프론트엔드 상태 관리**: 
  - Zustand 스토어에 개인정보 동의 관리 로직 통합
  - `checkPrivacyConsent()`, `givePrivacyConsent()` 메서드 구현
  - 게스트/로그인 사용자 구분하여 동의 상태 처리
- **UX 최적화**: 
  - 이미 동의한 사용자에게는 동의 UI를 표시하지 않음
  - 편집 모드에서는 개인정보 동의 절차 생략
  - 법적 컴플라이언스를 위한 최소한의 사용자 경험 제공
- **향후 기능 준비**: MyQRListScreen에 "약관 및 동의 내역" 메뉴 공간 확보
